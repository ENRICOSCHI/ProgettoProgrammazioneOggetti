...
namespace GiocoTestualeEsame.stanze
{
    public class ElencoStanze
    {
        ...
        public static Stanza primoPiano { get; } = new Stanza("primo piano", "sei al primo piano, qui c'è una vista fantastica... però guardando dalle finistre si vede una prigione, chissà se è possibile arrivarci?");
        public static Stanza salaGiochi { get; } = new Stanza("sala giochi", "sei nella sala giochi, però non si vede molto, è tutta buglia perchè è saltata la corrente e non è quindi possibile giocare a nessun arcade.");
        public static Stanza camera { get; } = new Stanza("camera", "sei in camera, non c'è molto apparte qualche abbigliamento e armature.");
